...
class RecintoFiscals extends Component
{
    use WithPagination;
    ...
    {
        return view('livewire.recinto_fiscals.recinto_fiscals', [
            'recinto' => $this->loadData()
        ]);
    }
    ...
    public function loadData()
    {
        if (!empty($this->search)) {
            ...
            $query = RecintoFiscal::where('valor', 'like', "%{$this->search}%")
                ->orWhere('codigo', 'like', "%{$this->search}%")
                ->orderBy('codigo', 'asc');

        } else {
            $query = RecintoFiscal::orderBy('id', 'asc');
        }
    ...
    protected function messages()
    {
        return [
            ...
            'valor.unique' => 'Ya existe el recinto fiscal',
    ...
    public function Store()
    {
        ...
        $data = RecintoFiscal::create([
            'codigo' => $this->codigo,
            'valor' => $this->valor,
            'status' => $this->status
        ]);

        $this->dispatch('noty', msg: 'Recinto Fiscal registrado con exito');
        $this->ResetInt();
    ...
    public function Update()
    {
        ...
        $data = RecintoFiscal::find($this->selected_id);
        $data->codigo = $this->codigo;
        $data->valor = $this->valor;
        $data->status = $this->status;
        $data->save();

        $this->dispatch('noty', msg: 'Recinto Fiscal Actualizado con exito');
        $this->ResetInt();
        $this->dispatch('close-modal');
    }
    ...
    public function destroy($id)
    {
        ...
        $data->delete();
        $this->resetPage();
        $this->dispatch('noty', msg: 'RECINTO FISCAL ELIMINADO CON ÉXITO');
    }
