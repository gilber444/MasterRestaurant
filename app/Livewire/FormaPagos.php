...
class FormaPagos extends Component
{
    use WithPagination;
    ...
    {
        return view('livewire.forma_pagos.forma_pagos', [
            'pago' => $this->loadData()
        ]);
    }
    ...
    public function loadData()
    {
        if (!empty($this->search)) {
            ...
            $query = FormaPago::where('valor', 'like', "%{$this->search}%")
                ->orWhere('codigo', 'like', "%{$this->search}%")
                ->orderBy('codigo', 'asc');

        } else {
            $query = FormaPago::orderBy('id', 'asc');
        }
    ...
    protected function messages()
    {
        return [
            ...
            'valor.required' => 'El nombre de la forma de pago es requerido',
            'valor.unique' => 'Ya existe la forma de pago',
            'valor.min'=> 'El nombre de la forma de pago debe tener mas de 3 caracteres',
    ...
    public function Store()
    {
        ...
        $data = FormaPago::create([
            'codigo' => $this->codigo,
            'valor' => $this->valor,
            'status' => $this->status
        ]);

        $this->dispatch('noty', msg: 'Forma de Pago registrado con exito');
        $this->ResetInt();
    ...
    public function Update()
    {
        ...
        $data = FormaPago::find($this->selected_id);
