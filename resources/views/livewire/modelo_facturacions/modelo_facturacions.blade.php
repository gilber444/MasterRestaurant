<div class="row">
    <div class="col-md-12">
        <div class="card height-equal">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-responsive-md table-hover">
                        ...
                        <tbody>
                            @forelse ($facturacion as $fac)
                                <tr>
                                    <td class="text-center">{{ $fac->codigo }}</td>
                                    <td class="text-center"> {{ $fac->valor }}</td>
                                    <td class="text-center">
                                        @if($fac->status === 'Activo')
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-pill" role="group">
                                            @can('Facturacion_Update')
                                                <button class="btn rounded-pill btn-icon btn-outline-warning waves-effect btn-sm"
                                                wire:click="Edit({{ $fac->id }})"><i class="ri-edit-box-line"></i>
                                                </button>
                                            @endcan
                                            @can('Facturacion_Destroy')
                                                <a href="#" onclick="confirmDestroy({{ $fac->id }})" class="btn rounded-pill btn-icon btn-outline-danger waves-effect btn-sm"><i class="ri-delete-bin-line"></i></a>
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
            ...
            @include('livewire.modelo_facturacions.forms')
        </div>
    </div>
</div>
