<div class="row">
    <div class="col-md-12">
        <div class="card height-equal">
                    ...
                    <table class="table table-responsive-md table-hover">
                        ...
                        <tbody>
                            @forelse ($marcas as $marca)
                                <tr>
                                    <td class="text-center" style="font-size: 0.8rem;">{{ $marca->id }}</td>
                                    <td class="text-center" style="font-size: 0.8rem;">{{ $marca->nombre }}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-pill gap-3" role="group" >
                                            @can('Productosmarcas_Update')
                                                <button class="btn rounded-pill btn-icon btn-outline-warning waves-effect btn-sm"
                                                wire:click="Edit({{ $marca->id }}, true)"><i class="ri-edit-box-line"></i>
                                                </button>
                                            @endcan
                                            @can('Productosmarcas_Destroy')
                                                <a href="#" onclick="confirmDestroy({{ $marca->id }})" class="btn rounded-pill btn-icon btn-outline-danger waves-effect btn-sm"><i class="ri-delete-bin-line"></i></a>
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
            ...
            @include('livewire.productos_marcas.forms')
