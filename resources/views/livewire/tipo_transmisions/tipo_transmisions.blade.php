<div class="row">
    <div class="col-md-12">
        <div class="card height-equal">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-responsive-md table-hover">
                        ...
                        <tbody>
                            @forelse ($transmision as $tr)
                                <tr>
                                    <td class="text-center">{{ $tr->codigo }}</td>
                                    <td class="text-center"> {{ $tr->valor }}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-pill" role="group">
                                            @can('Transmision_Update')
                                                <button class="btn rounded-pill btn-icon btn-outline-warning waves-effect btn-sm"
                                                wire:click="Edit({{ $tr->id }})"><i class="ri-edit-box-line"></i>
                                                </button>
                                            @endcan
                                            @can('Transmision_Destroy')
                                                <a href="#" onclick="confirmDestroy({{ $tr->id }})" class="btn rounded-pill btn-icon btn-outline-danger waves-effect btn-sm"><i class="ri-delete-bin-line"></i></a>
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
            ...
            @include('livewire.tipo_transmisions.forms')
        </div>
    </div>
</div>
