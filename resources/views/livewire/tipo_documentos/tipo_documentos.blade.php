<div class="row">
    <div class="col-md-12">
        <div class="card height-equal">
                        ...
                        <tbody>
                            @forelse ($documentos as $documento)
                                <tr>
                                    ...
                                    <td class="text-center"> {{ $documento->valor }}</td>
                                    <td class="text-center">
                                        @if($documento->status === 'Activo')
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-pill" role="group">
                                            @can('Documentos_Update')
                                                <button class="btn rounded-pill btn-icon btn-outline-warning waves-effect btn-sm"
                                                wire:click="Edit({{ $documento->id }})"><i class="ri-edit-box-line"></i>
                                                </button>
                                            @endcan
                                            @can('Documentos_Destroy')
                                                <a href="#" onclick="confirmDestroy({{ $documento->id }})" class="btn rounded-pill btn-icon btn-outline-danger waves-effect btn-sm"><i class="ri-delete-bin-line"></i></a>
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
            ...
            @include('livewire.tipo_documentos.forms')
        </div>
    </div>
</div>
